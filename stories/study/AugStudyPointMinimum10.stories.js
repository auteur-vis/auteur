...
export default {
  title: 'Aug/Study/Point/Minimum10',
};
...
export const ToStorybook = () => {
	...
	useEffect(() => {
		...
		let styles = {"line":{"stroke-width": "2px"},
					 "fill":{"fill": "black"},
					 "stroke":{"stroke-width": (d) => sizeScale(d.Revenue)}};

		newPointThreshold.current.updateStyles(styles);

		chart.current.chart(ref.current)
					.selection(points)
					.x("Store", xScale)
					.y("Revenue", yScale)
					.exclude({"name":["opacity", "text"]})
