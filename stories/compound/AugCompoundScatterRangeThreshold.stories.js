...
export default {
  title: 'Aug/Compound/Scatter/RangeThreshold',
};
...
export const ToStorybook = () => {
	...
	const ref = useRef("RangeThreshold");
	const chart = useRef(new Draft());
	const newRange = useRef(new Range("Flavor", ["lowerBound", "upperBound"], "closed", style));
	...
	useEffect(() => {
		...
		newRange.current.updateStyles({"rect":{"opacity":0.035}});
		...
		chart.current.chart(ref.current)
					...
					.x("Aroma", xScale)
					.y("Flavor", yScale)
					.exclude({"name":["stroke", "label", "regression"]})
					.augment(newXThreshold.current.intersect(newRange.current));
