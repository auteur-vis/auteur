...
export default class Draught {
	...
	// Handle augmentations for bar charts
	handleBarAugs(augmentations, svgElement, xScale, yScale, xMin, xMax, yMin, yMax) {

		let allBars = svgElement.select("#mark")
								  .selectAll(".barrect")

		for (let [key, val] of Object.entries(this._defaultStyles["bar"])) {
			allBars.attr(key, val);
		}
		...
		for (let i = 0; i < augmentations.length; i++) {
			...
			if (aug.type === "line") {
				...
				if (!aug.style) {
					...
					console.log(augIndex);
					let augBars = svgElement.select("#mark")
											  .selectAll(".barrect")
											  .filter(function (d, i) {
											  	if (chartIndex === "default") {
											  		return augIndex.includes(i)
	...
	render() {
		...
		if (this._mark === "point") {
			...
			let scatterpoints = svgElement.select("#mark")
				...
				.attr("cx", d => this._xType === "categorical" ? xScale(d[this._xVar]) + xScale.bandwidth() / 2 : xScale(d[this._xVar]))
				.attr("cy", d => this._yType === "categorical" ? yScale(d[this._yVar]) + yScale.bandwidth() / 2 : yScale(d[this._yVar]))
				.attr("r", 3)

			this.handlePointAugs(this._augs, svgElement, xScale, yScale, layout.marginLeft, layout.width - layout.marginRight, layout.height - layout.marginBottom, layout.marginTop);
			...
			let barrects = svgElement.select("#mark")
				...
				.join("rect")
				.attr("class", "barrect")
				.attr("x", d => this._xType === "categorical" ? xScale(d[this._xVar]) + 1 : layout.marginLeft)
				.attr("y", d => this._yType === "categorical" ? yScale(d[this._yVar]) + 1 : yScale(d[this._yVar]))
				.attr("width", d => this._xType === "categorical" ? xScale.bandwidth() - 2 : xScale(d[this._xVar]) - layout.marginLeft)
				.attr("height", d => this._yType === "categorical" ? yScale.bandwidth() - 2 : layout.height - layout.marginBottom - yScale(d[this._yVar]))

			this.handleBarAugs(this._augs, svgElement, xScale, yScale, layout.marginLeft, layout.width - layout.marginRight, layout.height - layout.marginBottom, layout.marginTop);
